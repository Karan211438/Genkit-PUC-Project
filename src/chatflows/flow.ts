...
import { getPluginTraceDetails, retrieve_entity_metadata } from "../backendfunction/function.js";
...
export const getPluginTraceLogFilter =ai.defineFlow(
  ...
  async (input) => {
    const result =  await getPluginTraceDetails();
    const a = await NormalChatFlow({ prompt: input.prompt +result});
    console.log("Final Answer:", a);
    return a;
  }
)
export const RetrieveEntityMetadataFlow = ai.defineFlow(
  ...
  async (input) => {
    const { output } = await ai.generate({
     ...
     const result =  await retrieve_entity_metadata(output.partialmetadataurl);
